...
public class NotificationService {
    ...
    @Transactional(readOnly = true) public List<EventInvitationResponse> eventPending(String email) { Long id = user(email).getId(); return eventInvitations.findByInviteeIdAndStatusOrderByCreatedAtDesc(id, "PENDING").stream().filter(i -> i.getEvent().getCreatedBy().getId().equals(i.getInviter().getId())).map(EventInvitationResponse::of).toList(); }
    @Transactional(readOnly = true) public List<EventInvitationResponse> allEvents(String email) { return eventInvitations.findByInviteeIdOrderByCreatedAtDesc(user(email).getId()).stream().map(EventInvitationResponse::of).toList(); }
    ...
    @Transactional(readOnly = true) public NotificationCountResponse count(String email) { Long id = user(email).getId(); long groups = invitations.countByInviteeIdAndStatus(id, "PENDING"); long eventInvites = eventInvitations.findByInviteeIdAndStatusOrderByCreatedAtDesc(id, "PENDING").stream().filter(i -> i.getEvent().getCreatedBy().getId().equals(i.getInviter().getId())).count(); long eventRequests = eventInvitations.findByInviteeIdAndStatusOrderByCreatedAtDesc(id, "PENDING").stream().filter(i -> i.getEvent().getCreatedBy().getId().equals(id) && !i.getInviter().getId().equals(id)).count(); long events = eventInvites + eventRequests; return new NotificationCountResponse(groups, events, groups + events); }
