...
public class ChatWebSocketHandler extends TextWebSocketHandler {
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        boolean delivered = rooms.getOrDefault(room, Set.of()).stream().anyMatch(peer -> peer != session && peer.isOpen());
        ...
        if (delivered) {
            response = room.startsWith("group/")
                    ? groups.markDelivered(((com.reseausocial.group.dto.MessageResponse) response).id(), email)
                    : events.markDelivered(((com.reseausocial.group.dto.EventMessageResponse) response).id(), email);
        }
